...
class ArgIn:
    ...
    def __init__ ( self, mode="", decoder= "", batch_size= None, data_dir="",fast=None,line_mode =None, img_file="",early_stopping=None, dump= None ):
        if mode:
            self.mode = mode
        if decoder:
...
def infer(model: Model, fn_img: Path):
    ...
    if fn_img is Path: 
        print(str(fn_img)[1:])
        path_tail = os.path.split(fn_img)
        print(path_tail[1])
        img = cv2.imread(str(fn_img)[1:], cv2.IMREAD_GRAYSCALE)
    else:
        img = np.asarray(fn_img)
...
def predix( mode="", decoder= "", batch_size= None, data_dir="",fast=None,line_mode =None, img_file="",early_stopping=None, dump= None ):
    """Main function."""

    #parse arguments and set CTC decoder
    # args = parse_args(self, name="", decoder= "", batch_size= None, data_dir="",fast=None,line_mode =None, img_file="",early_stopping=None, dump= None)
    ...
    args = ArgIn( mode, decoder, batch_size, data_dir,fast,line_mode , img_file,early_stopping, dump)
    decoder_mapping = {'bestpath': DecoderType.BestPath,
                     'beamsearch': DecoderType.BeamSearch,
                     'wordbeamsearch': DecoderType.WordBeamSearch}
    decoder_type = decoder_mapping[args.decoder]
